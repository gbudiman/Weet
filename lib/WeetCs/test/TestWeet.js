  ...
  contract('chain manipulation', async() => {
    ...
    it('should retrieve weets correctly', async() => {
      for (const weet of weets) {
        ...
        let content = r[3]
        let is_evaluated = r[4]
        let is_published = r[5]
    ...
    it('should publish weet correctly', async() => {
      let weet = weets[0]
      await instance.publish_weet(weet.weet_id, weet.weeter_id, weet.timestamp, weet.weet, true, { from: admin })

      let cweet = await instance.get_weet.call(weet.weet_id)
      let is_evaluated = cweet[4]
      let is_published = cweet[5]
      assert(is_evaluated)
      assert(is_published)
    })

    it('should reject evaluating already-evaluated weet', async() => {
      let weet = weets[2]
      await instance.publish_weet(weet.weet_id, weet.weeter_id, weet.timestamp, weet.weet, false, { from: admin })
      ...
      try {
        await instance.publish_weet(weet.weet_id, weet.weeter_id, weet.timestamp, weet.weet, true, { from: admin })
        assert.fail('Expecting rejection due to evaluated Weet')
      } catch(error) {
        let cweet = await instance.get_weet.call(weet.weet_id)
        let is_evaluated = cweet[4]
        let is_published = cweet[5]
    ...
    it('should reject publishing from non-admin', async() => {
      ...
      try {
        await instance.publish_weet(weet.weet_id, weet.weeter_id, weet.timestamp, weet.weet, true, { from: dummy })
